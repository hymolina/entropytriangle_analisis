...
def training(K,basedir,n_epochs=10,batch_length=250,numbins=32,database='MNIST',start_epoch=0,num_gpus=1):
    ...
    with tf.device(BASE_DEVICE):
        ...
        if ( buildModel ):
            ...
            enc_layer1=Dense(dim_layers[1],activation='relu')(input_image)
            enc_layer2=Dense(dim_layers[2],activation='relu')(enc_layer1)
            enc_layer3=Dense(dim_layers[3],activation='relu')(enc_layer2)
            enc_layer4=Dense(dim_layers[4],activation='relu')(enc_layer3)


            # Decoder definition


            dec_layer3=Dense(dim_layers[3],activation='relu')(enc_layer4)
            dec_layer2=Dense(dim_layers[2],activation='relu')(dec_layer3)
            dec_layer1=Dense(dim_layers[1],activation='relu')(dec_layer2)
    ...
    for ind_epoch in range(start_sample, n_epochs): 
        ...
        # for ind_batch in range(0, 3):
            ...
            # Almacenamos el accuracy y loss de cada fichero del batch.
            ...
            X_out_1 = intermediate_layer_model_1.predict_on_batch(X_train)
            X_out_2 = intermediate_layer_model_2.predict_on_batch(X_train)
            X_out_3 = intermediate_layer_model_3.predict_on_batch(X_train)
            X_out_Z = intermediate_layer_model_Z.predict_on_batch(X_train)
            X_out_3p = intermediate_layer_model_3p.predict_on_batch(X_train)
            X_out_2p = intermediate_layer_model_2p.predict_on_batch(X_train)
            X_out_1p = intermediate_layer_model_1p.predict_on_batch(X_train)
            X_out_0p = execution_model.predict_on_batch(X_train)
            elapsed_time = time.time() - start_time
            #print ("Tiempo de prediccion: {0}".format(elapsed_time))

            start_time = time.time()
            DF_x_out_1=discretizer.parallelize_discretization(pd.DataFrame(X_out_1))
            DF_x_out_2=discretizer.parallelize_discretization(pd.DataFrame(X_out_2))
            DF_x_out_3=discretizer.parallelize_discretization(pd.DataFrame(X_out_3))
            DF_x_out_Z=discretizer.parallelize_discretization(pd.DataFrame(X_out_Z))
            DF_x_out_3p=discretizer.parallelize_discretization(pd.DataFrame(X_out_3p))
            DF_x_out_2p=discretizer.parallelize_discretization(pd.DataFrame(X_out_2p))
            DF_x_out_1p=discretizer.parallelize_discretization(pd.DataFrame(X_out_1p))
            DF_x_out_0p=discretizer.parallelize_discretization(pd.DataFrame(X_out_0p))
            ...
            # X_3->X_3'
            # X->Z
            spool_data=np.array([(X_discrete,DF_x_out_0p),
                                (X_discrete,DF_x_out_Z),
                                (DF_x_out_1,DF_x_out_1p),
                                (DF_x_out_2,DF_x_out_2p),
                                (DF_x_out_3,DF_x_out_3p),
                                ],dtype=object)
            ...
            buffer["loss"]          = loss
            buffer["input"]         = pd.Series(["X","X","X","X","X","X","X1","X1","X1","X2","X2","X2","X3","X3","X3"],
                                                dtype="category",
                                                index=buffer.index)
            buffer["output"]        = pd.Series(["Xp","Xp","Xp","Z","Z","Z","X1p","X1p","X1p","X2p","X2p","X2p","X3p","X3p","X3p"],
                                                dtype="category",
                                                index=buffer.index)                     
